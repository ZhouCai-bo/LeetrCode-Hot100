# 2020/04/24 hot-100完结撒花
这一百道题前后做了两个月吧，中间穿插着实习笔试面试，总算是搞定了，从刚开始的两眼一抹黑，硬着头做到现在也算是看到题之后能有一些思路。</br>
实习面试了好几家到现在也没有收到正式offer，今年还是挺难的，现在每天早上刷题，下午晚上看书，希望秋招的时候能去自己想去的地方，过自己想过的生活，诸位加油。
...
# 399. 除法求值
第一次用（带权）并查集。。。<br/>
* 简单来说就是不断通过给定的结果构造边，然后使用压缩路算法使互有联系的边拥有同一个根<br/>
* 构造完并查集之后，取出queries中需要找到结果的两个节点，如果他们在拥有同一个根，则通过除法法则得到其结果为map[b].second/map[a].second；若他们没有同样的根，则找不到他们之间的倍数关系，结果为-1.0。

# 406. 根据身高重建队列
“矮个子对于高个子来说是看不见的”哈哈哈哈啊哈哈<br/>
* 由上述可得，先把高个子按照k排序，在依次将矮的按照k插入到已排序队伍中，直接使用k作为插入的索引即可。
* 可以使用list容器，因为我们要频繁的进行插入操作，注意list的迭代器是不能随机访问的，所以要使用advance函数来移动迭代器。

# 416. 分割等和子集
复习了一遍背包问题<br/>
背包问题有两个难点：
* 状态转移方程：定义dp[j]表示当前体积为j时的最大价值，类比本题为当前和为j的bool值；那么当前和为j的来源有：
   * 1.本来和为j就是true的；
...
# 448. 找到所有数组中消失的数字
抽屉定理，题目中说有的数字出现了两次，而且有`n=>nums[i]>=1`，所以直接把每一个数字填在`a[i]-1`的位置，若果当前位置已经是正确的就跳过，最后再遍历一遍数组，如果某个位置的数字不是`i+1`，就吧`i+1`添加进结果数组。
...
# 581. 最短无序连续子数组
算是某种暴力解吧，用一个滑动窗口，先确定右边界，再确定左边界，同时因为要用到窗口中的最大值和最小值，所以维护两个数组maxs和mins，分别代表从第0个元素至第i个元素之间的最大值，以及从第n-1个元素到第i个元素的最小值。</br>
* 确定右边界的时候，将无序元素或者有序但是小于窗口最大值的元素加入窗口；
* 确定左边界的时候，从第0个元素开始遍历，将小于等于窗口最小值的元素弹出窗口，同时若元素等于窗口最小值，更新窗口最小值。
...
# 739. 每日温度
思路总是差那么一点。。。</br>
维护最小栈，存储温度按降序排列的天数索引，遇到比当前栈顶天温度高的时候，出栈，结果数组中更新栈顶天的值为`i - st.top()`，最后可能栈未空，需要加一次循环出栈。
